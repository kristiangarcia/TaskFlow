...
SELECT
    ...
    t.fecha_creacion,
    COUNT(a.id_asignacion) AS total_usuarios_asignados,
    COALESCE(SUM(a.horas_asignadas), 0) AS total_horas_asignadas,
    CASE
        WHEN t.fecha_limite < CURRENT_DATE AND t.estado NOT IN ('completada', 'retrasada') THEN true
        ELSE false
    END AS esta_vencida
FROM tareas t
...
-- Vista: Mis tareas (para empleados) - Implementa la cardinalidad 1:N desde USUARIOS
CREATE VIEW vista_mis_tareas AS
SELECT
    u.id_usuario,
    u.nombre_completo,
    t.id_tarea,
    ...
    a.fecha_asignacion
FROM usuarios u
INNER JOIN asignaciones a ON u.id_usuario = a.usuario_id
INNER JOIN tareas t ON a.tarea_id = t.id_tarea
WHERE u.activo = TRUE;

-- Vista: Usuarios asignados a cada tarea - Implementa la cardinalidad 1:M desde TAREAS
CREATE VIEW vista_asignaciones_por_tarea AS
SELECT
    t.id_tarea,
    ...
    a.notas
FROM tareas t
INNER JOIN asignaciones a ON t.id_tarea = a.tarea_id
...
SELECT
    ...
    (SELECT COUNT(*) FROM usuarios WHERE activo = TRUE AND rol = 'empleado') AS empleados_activos,
    (SELECT COUNT(*) FROM tareas WHERE estado IN ('abierta', 'en_progreso')) AS tareas_activas,
    (SELECT COUNT(*) FROM tareas WHERE estado = 'completada') AS tareas_completadas,
    (SELECT COUNT(*) FROM tareas WHERE estado = 'retrasada') AS tareas_retrasadas,
    (SELECT COUNT(*) FROM tareas WHERE fecha_limite < CURRENT_DATE AND estado NOT IN ('completada', 'retrasada')) AS tareas_vencidas,
    (SELECT COUNT(DISTINCT proyecto_categoria) FROM tareas) AS categorias_proyecto;
...
-- SELECT * FROM tareas
-- WHERE fecha_limite BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days'
-- AND estado NOT IN ('completada', 'retrasada')
-- ORDER BY fecha_limite ASC;

-- Obtener usuarios disponibles (no asignados a una tarea específica)
-- SELECT u.* FROM usuarios u
-- WHERE u.activo = TRUE
-- AND u.id_usuario NOT IN (
--     SELECT usuario_id FROM asignaciones WHERE tarea_id = 1
-- );
...
--     u.nombre_completo,
--     u.rol,
--     COUNT(a.id_asignacion) AS num_tareas_asignadas,
--     SUM(a.horas_asignadas) AS total_horas_asignadas,
--     SUM(CASE WHEN a.completado THEN 1 ELSE 0 END) AS tareas_completadas
-- FROM usuarios u
